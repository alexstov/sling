...
func sendRun(cmd *cobra.Command, args []string) {
	...
	case emul.RepeatReq, emul.MultiReq:
		...
		// Gouroutine to prepare all requests
		go prepareRequests(in, &sendArgs, &wg)

		// Gouroutine to send all requests.
	...
	if flag, ok := flagmap[LogHis]; ok {
		...
		if logHis {
			metrics.LogScaledOnce(reg, time.Millisecond, logger.GetLogger())
		}
	}
	...
	if flag, ok := flagmap[ConHis]; ok {
		...
		if conHis {
			metrics.LogScaledOnce(reg, time.Millisecond, em.Consoler.GetLogger())
		}
	}

	// Log and print commnad execution result.
	if err != nil {
