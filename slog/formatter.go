...
func (f *SlingTextFormatter) init(entry *logrus.Entry) {
...
func (f *SlingTextFormatter) checkIfTerminal(w io.Writer) bool {
	switch v := w.(type) {
	case *os.File:
...
func (f *SlingTextFormatter) printColored(b *bytes.Buffer, entry *logrus.Entry, keys []string, timestampFormat string, colorScheme *compiledColorScheme) {
...
func (f *SlingTextFormatter) needsQuoting(text string) bool {
...
func (f *SlingTextFormatter) appendKeyValue(b *bytes.Buffer, key string, value interface{}, appendSpace bool) {
	b.WriteString(key)
	b.WriteByte('=')
...
func (f *SlingTextFormatter) appendValue(b *bytes.Buffer, value interface{}) {
